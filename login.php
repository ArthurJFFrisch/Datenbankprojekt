    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (strlen($_POST["username"]) >= 3) {
            ...
            if ($db instanceof Throwable) {
                ...
                $sql = $db->prepare("SELECT password, displayname FROM user WHERE username=? AND verified=1");
                $sql->bind_param("s", $_POST['username']);
                $sql->execute();
                $result = $sql->get_result();
                $row = $result->fetch_assoc();
                if (isset($row["password"]) && password_verify($_POST['password'], (string)$row["password"])){
                    $token = generate_login_token($_POST['username']);
                    $_SESSION["username"] = check_login_token($token);
